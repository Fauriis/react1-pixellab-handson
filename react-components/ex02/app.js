...
console.warn(
  'Folosind obiectul person si redyce, afiseaza in consola un string care contine skillurile de pe pozitiile pare ale arrayului, separate prin virgula',
);

const string1 = person.skills.reduce((string1, skill, index, skills) => {
  const punctuation = index === skills.length - 2 ? '' : ', ';

  if (index % 2 === 0) {
    string1 += `${skill}${punctuation}`;
  }
...
const string3 = person.friends
  .reduce((string3, { age, name }) => {
    string3 += `Intre ${person.name} si ${name} este o diferente de ${Math.abs(
      person.age - age,
    )} ani. `;
